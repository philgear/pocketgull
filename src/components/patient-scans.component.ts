    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
      @for(scan of scans(); track scan.id) {
          ...
          @if (scan.imageUrl) {
             <div class="h-40 bg-gray-100 dark:bg-zinc-950 relative overflow-hidden flex-shrink-0 border-b border-gray-100 dark:border-zinc-800">
               ...
               <div class="absolute top-2 right-2 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider backdrop-blur-md shadow-sm border"
                 [class.bg-brand-green-50]="scan.status === 'Normal'" [class.text-brand-green-700]="scan.status === 'Normal'" [class.border-brand-green-200]="scan.status === 'Normal'" [ngClass]="{'dark:bg-brand-green-900/40': scan.status === 'Normal'}" [ngClass]="{'dark:text-brand-green-400': scan.status === 'Normal'}" [ngClass]="{'dark:border-brand-green-800/50': scan.status === 'Normal'}"
                 [class.bg-brand-red-50]="scan.status === 'Abnormal'" [class.text-brand-red-700]="scan.status === 'Abnormal'" [class.border-brand-red-200]="scan.status === 'Abnormal'" [ngClass]="{'dark:bg-brand-red-900/40': scan.status === 'Abnormal'}" [ngClass]="{'dark:text-brand-red-400': scan.status === 'Abnormal'}" [ngClass]="{'dark:border-brand-red-800/50': scan.status === 'Abnormal'}"
                 [class.bg-brand-amber-50]="scan.status === 'Pending'" [class.text-brand-amber-700]="scan.status === 'Pending'" [class.border-brand-amber-200]="scan.status === 'Pending'" [ngClass]="{'dark:bg-brand-amber-900/40': scan.status === 'Pending'}" [ngClass]="{'dark:text-brand-amber-400': scan.status === 'Pending'}" [ngClass]="{'dark:border-brand-amber-800/50': scan.status === 'Pending'}"
        ...
        <div class="col-span-full py-12 flex flex-col items-center justify-center text-gray-500 dark:text-zinc-400 border border-dashed border-gray-200 dark:border-zinc-800 rounded-lg bg-gray-50/50 dark:bg-zinc-900/50">
           ...
           <p class="text-[10px] uppercase tracking-widest text-gray-500 mt-1">IPatient records empty</p>
        </div>
      }
    </div>
  `
