      ...
      <div class="px-5 py-4 border-b border-gray-100 dark:border-slate-800/60 bg-gradient-to-r from-slate-900 to-slate-800 flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-[9px] font-medium text-sky-400/80 uppercase tracking-widest">DOC / Coma Sensory Stimulation</p>
          </div>
        </div>
        @if (docService.session()) {
          <span class="text-[9px] px-2.5 py-1 rounded-full bg-sky-500/15 text-sky-400 font-bold uppercase tracking-wider border border-sky-500/20">
            Protocol Active
          </span>
        }
      </div>
        ...
        @if (!docService.session()) {
          ...
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-3">
              <div class="space-y-1.5">
                <label class="text-[9px] font-bold uppercase tracking-widest text-slate-400">GCS Score</label>
                ...
                <p class="text-[9px] text-slate-500 italic">{{ gcsInterpretation() }}</p>
              </div>

              <div class="space-y-1.5">
                <label class="text-[9px] font-bold uppercase tracking-widest text-slate-400">DOC Level</label>
            ...
            <div class="grid grid-cols-2 gap-3">
              <div class="space-y-1.5">
                <label class="text-[9px] font-bold uppercase tracking-widest text-slate-400">Etiology</label>
              ...
              <div class="space-y-1.5">
                <label class="text-[9px] font-bold uppercase tracking-widest text-slate-400">Days Post-Onset</label>
            ...
            <!-- Preferred Music -->
            <div class="space-y-1.5">
              <label class="text-[9px] font-bold uppercase tracking-widest text-slate-400">Preferred Music (from family)</label>
            ...
            <div class="grid grid-cols-2 gap-2">
              @for (flag of checkboxFlags; track flag.key) {
                <label class="flex items-center gap-2 cursor-pointer group">
                  ...
                  <span class="text-[10px] text-slate-400 group-hover:text-slate-200 transition-colors leading-tight">{{ flag.label }}</span>
                </label>
              }
            </div>
            ...
            <button (click)="generate()"
                    class="w-full py-2.5 px-4 rounded-xl font-bold uppercase tracking-wider text-[11px] bg-gradient-to-r from-sky-600 to-blue-600 text-white shadow-lg shadow-sky-500/20 hover:shadow-sky-500/40 transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer">
          ...
          @if (docService.session(); as sess) {
            ...
            <div class="p-3 rounded-lg bg-sky-500/[0.06] border border-sky-500/20">
              <p class="text-[9px] font-bold uppercase tracking-widest text-sky-400 mb-1">Clinician Note</p>
              ...
              <div class="flex items-center gap-4 mt-2 pt-2 border-t border-sky-500/10">
                <div class="text-center">
                  <p class="text-[8px] text-slate-500 uppercase tracking-widest">Total</p>
                  <p class="text-sm font-extrabold text-sky-400">{{ sess.totalDurationMin }} min</p>
                </div>
                <div class="text-center">
                  <p class="text-[8px] text-slate-500 uppercase tracking-widest">Sessions/Day</p>
                  <p class="text-sm font-extrabold text-blue-400">{{ sess.sessionsPerDay }}×</p>
                </div>
                <div class="text-center">
                  <p class="text-[8px] text-slate-500 uppercase tracking-widest">Blocks</p>
            ...
            @if (sess.safety_warnings.length > 0) {
              <div class="p-3 rounded-lg bg-red-500/[0.05] border border-red-500/20">
                <p class="text-[9px] font-bold uppercase tracking-widest text-red-400 mb-1.5">Safety Advisories</p>
                @for (w of sess.safety_warnings; track w) {
                  <p class="text-[10px] text-red-300/80 leading-snug mb-0.5">{{ w }}</p>
                }
              </div>
            }
            ...
            <!-- Schedule Blocks -->
            <div class="space-y-2">
              <p class="text-[9px] font-bold uppercase tracking-widest text-slate-500">Session Schedule</p>
                ...
                <div class="rounded-lg border overflow-hidden" [class]="blockBorderClass(block)">
                  <div class="px-3 py-2 flex items-center justify-between" [class]="blockHeaderClass(block)">
                    <div class="flex items-center gap-2">
                      ...
                      <span class="text-[10px] font-bold uppercase tracking-wider" [class]="blockTitleColor(block)">
                        {{ block.label }}
                      </span>
                      @if (block.frequencyHz) {
                        <span class="text-[8px] px-1.5 py-0.5 rounded bg-sky-500/20 text-sky-400 font-bold">{{ block.frequencyHz }} Hz</span>
                      }
                    </div>
                    <span class="text-[9px] font-bold text-slate-400">{{ block.durationMin }} min</span>
                  </div>
                  <div class="px-3 py-2.5 space-y-1.5">
                    <p class="text-[10px] text-slate-200 leading-relaxed font-medium">{{ block.instruction }}</p>
                    <p class="text-[9px] text-slate-500 italic leading-relaxed">{{ block.rationale }}</p>
                  </div>
                </div>
              }
            </div>
            ...
            <div class="p-3 rounded-lg bg-violet-500/[0.05] border border-violet-500/20">
              <p class="text-[9px] font-bold uppercase tracking-widest text-violet-400 mb-2">Family Guidance</p>
              @for (g of sess.family_guidance; track g) {
                <p class="text-[10px] text-slate-300 leading-snug mb-1.5">{{ g }}</p>
              }
            </div>

            <!-- Evidence References (collapsed) -->
            <details class="group">
              <summary class="text-[9px] font-bold uppercase tracking-widest text-slate-500 cursor-pointer hover:text-slate-300 transition-colors list-none flex items-center gap-1.5">
              ...
              <div class="mt-2 space-y-1 pl-5 border-l border-slate-800">
                @for (ref of sess.evidence_references; track ref) {
                  <p class="text-[9px] text-slate-500 leading-snug">{{ ref }}</p>
                }
              </div>
            </details>
            ...
            <!-- Reset -->
            <button (click)="reset()"
                    class="w-full py-2 rounded-xl font-bold uppercase tracking-wider text-[10px] border border-slate-700/50 text-slate-500 hover:text-slate-300 hover:border-slate-600 transition-all cursor-pointer">
              Reset / New Patient
            </button>
