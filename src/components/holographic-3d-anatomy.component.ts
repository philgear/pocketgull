...
export class Holographic3DAnatomyComponent implements AfterViewInit, OnDestroy {
  ...
  private buildProceduralSkeletalMesh() {
    ...
    const boneMaterial = new THREE.MeshBasicMaterial({ color: 0x00e5ff, wireframe: true });
    ...
    // Skull sphere
    const skull = new THREE.Mesh(new THREE.SphereGeometry(0.28, 16, 16), boneMaterial);
  ...
  private buildTCMMeridians() {
    ...
    const acupointMat = new THREE.MeshBasicMaterial({ color: 0x10b981 });
    const st36 = new THREE.Mesh(new THREE.SphereGeometry(0.04, 8, 8), acupointMat);
  ...
  private buildAyurvedicChakras() {
    ...
    colors.forEach((col, idx) => {
      ...
      const sphere = new THREE.Mesh(new THREE.SphereGeometry(0.06 + idx * 0.005, 12, 12), mat);
