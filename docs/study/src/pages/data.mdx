...
A `RandomForestClassifier` is trained on engineered clinical features extracted from standard vitals and demographics:
*   **Heart Rate (HR)** (BPM)
...
Raw classification scores are calibrated using a `CalibratedClassifierCV` wrapper (Brier score evaluated) to align raw model thresholds directly to standard diagnostic probability scores (Stable vs. Critical Event).

### 3. Clinical Feature Governance
To comply with medical safety guidelines, features are validated to ensure that Oxygen Saturation (`spo2`) remains the highest weighted predictor in triage classifications, preventing false negatives on acute respiratory events.
