...
test.describe('Somatic Grounding & Anti-Gravity Physics Suite', () => {
  ...
  test('should open Zamecznik Calibrator canvas, toggle Anti-Gravity Mode, and adjust gravity scaling', async ({ page }) => {
    ...
    await page.waitForFunction(() => typeof (window as any).__openZamecznikCanvas === 'function', { timeout: 20000 });
    ...
    await expect(groundingHeading).toBeVisible({ timeout: 15000 });

    // Click Anti-Gravity toggle button
    const antiGBtn = page.locator('#antigravity-toggle-btn');
    await expect(antiGBtn).toBeVisible({ timeout: 10000 });
    await antiGBtn.click({ force: true });
    await expect(antiGBtn).toContainText('ON');
    ...
    await expect(scale05Btn).toHaveClass(/text-emerald-400/, { timeout: 5000 });

    const scale20Btn = page.locator('#scale-20-btn');
    await scale20Btn.evaluate((el: HTMLElement) => el.click());
    await expect(scale20Btn).toHaveClass(/text-emerald-400/, { timeout: 5000 });
    ...
    await expect(groundingHeading).not.toBeVisible({ timeout: 5000 });
  });
});
